...
const timelineDotVariants = cva(
  'col-start-2 col-end-3 row-start-1 row-end-1 flex size-4 items-center justify-center rounded-full border border-current text-blue-600',
  {
    variants: {
      status: {
        default: '[&>svg]:hidden',
        ...
        done: 'bg-blue-500 [&>.lucide-check]:text-background [&>svg:not(.lucide-check)]:hidden',
        error:
...
const TimelineDot = React.forwardRef<HTMLDivElement, TimelineDotProps>(
  ({ className, status, num, customIcon, ...props }, ref) => (
    <div
      role="status"
      className={cn(
        'timeline-dot',
    ...
    >
      ...
      <X className="size-3 bg-blue-500" />
      {customIcon}
    </div>
  ),
)
...
const TimelineContent = React.forwardRef<HTMLParagraphElement, TimelineConent>(
  ({ className, side, ...props }, ref) => (
    <p
      className={cn(timelineContentVariants({ side }), className, 'text-black')}
      ref={ref}
      {...props}
    />
  ),
